...
export default function Collapse(props) {
	// on declare les  variales d'etat ci dessous  
	const [toggle, setToggle] = useState(false); // je definie l'etat du bascule (et false par défaut)
	...
	const toggleState = () => {
		...
		setToggle(!toggle); // (!toggle= l'inverse de toggle c-a-d il va prendre la valeur de setToggle)
	};

	const refHeight = useRef(); //récupère et conserve la valeur de hauteur du collapse(effrondrement) déplié
	...
	return (
		...
		<div className={`collapse ${props.aboutStyle}`}>
			<div onClick={toggleState}// quand je click sur oneclick jappelle  toglesatate a la ligne 9 elle modifie puis le passe a l'inverse de la fonction toggle
				className="collapse__visible">
				<h2>{props.aboutTitle}</h2>
				<img
					className={toggle ? "chevron rotated" : "chevron"} //?=est ce que toggle est true affiche chevon qui tourne sinon chevron
					src={Chevron}
					alt="chevron"
				/>
			</div>
